...
interface BlogState {
  ...
  getAllBlogs: () => void;
  getAllUserBlogs: (userId: any) => void
...
const BlogStore = (set: any): BlogState => ({
  ...
  getAllUserBlogs: async (userId:any) => {
    try {
      const res = await fetch(`${domain}/blog/user-blogs`, {
        method: "GET",
        headers: {
          "content-type": "application/json",
          // Assuming you're sending the user ID in the request headers
          "user-id": userId,
        },
      });
      const userBlogsData = await res.json();
      ...
      return { userBlogs: userBlogsData };
    } catch (error: any) {
      console.log(error);
    }
  },
